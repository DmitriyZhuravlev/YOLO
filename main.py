...
class RTDETR:
    ...
    def draw_detections(self, box, score, class_id):
        ...
        color = self.color_palette[class_id % 80]

        # Draw the bounding box on the image
        cv2.rectangle(self.img, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)

        # Create the label text with class name and score
        label = f"{self.classes[class_id % 80]}: {score:.2f}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="yolo11m.onnx", help="Path to the ONNX model file.")
    ...
    check_requirements("onnxruntime-gpu" if False else "onnxruntime")
